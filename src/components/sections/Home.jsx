...
const journal = [
  {
    ...
    purpose: "After building my own C compiler, I wanted to understand how the hardware understands assembly code. That curiosity pushed me deep into computer architecture, instruction sets, and CPU design. I wanted to build a computer using the most basic components available to me, which were the CMOS 74HC logic ICs + 1 EEPROM. My goal was to understand instruction decoding, data flow, ALU design, bus architecture, and how a CPU truly works at the gate and register level. Along the way I ended up designing my own ISA, experimenting with memory addressing constraints, and learning far more about timing, latches, and hardware debugging than I expected.",
  ...
  },
  {
    ...
    purpose: "As part of my Computer Architecture class, I worked on a C compiler project—but most of the core pieces (lexer, parser, syntax tree) were provided. We ended up only writing grammar for the most part. I wanted to understand how a real compiler works from the ground up, so I decided to build my own C compiler in C and x86-64 assembly. The goal is to make SecC a self-hosting compiler—capable of compiling its own source code—while learning all the internals of compilers, parsing, semantic analysis, code generation, and toolchain design.",
  ...
  },
  {
    ...
    purpose: "I wanted to build a keyboard that was fully mine—designed, engineered, and customized down to the firmware. Instead of assembling a kit, I decided to create one from scratch: designing the PCB, soldering every component, modeling and 3D printing the case and keycaps, and writing the firmware that drives the whole system. The goal was to learn PCB design end-to-end, understand keyboard matrix scanning at the electrical/firmware level, and explore QMK customization beyond the typical configuration.",
